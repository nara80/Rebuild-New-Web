...
| Product page configurators | ✅ Built (8/27 products) | `public/js/product-configurator.js` — shared configurator on 8 product detail pages (4 fitted + 2 flat + 2 encasement). Auto-detects product type from URL path. Standard size dropdown → parse W×L×D → formula. Custom W×L×D → formula. Unit toggle (cm/inch). Full quote popup flow: [Custom Size] → [Custom Quote] → modal form → POST `/api/quote` → Resend email. |
| Product detail pages enhanced | ✅ | `standard-fitted-sheet`: fabric badge → dropdown + external JS reference. `dorm-fitted-sheet`: same upgrade. `deep-pocket-fitted-sheet`: full configurator panel. `rv-truck-fitted-sheet`: inline script replaced with shared configurator, CloudSoft-only, 45% margin. `flat-sheet-standard` + `flat-sheet-extra-deep-pocket`: full configurator with flat sheet formula. `mattress-encasement-general` + `rv-truck-mattress-encasement`: full configurator with TPU encasement formula, inline scripts replaced with shared configurator, `.tag-pill` CSS added, CTA labels unified. |
| 17 products await configurator | ⏸️ Pending | Marine/V-Berth, Family, Pet Owner fitted sheets; 6 duvet covers; 3 pillowcases; 5 mattress protectors; 1 pillow protector — currently use legacy placeholder pricing. |
| 2 products skip configurator | ✅ Verified | BedBridge Connector + Bed Lifter (fixed-price accessories, no custom dimensions). |
...
| AGENTS.md + Framework.md updated | ✅ | 27-product table, configurator pricing status (8 live / 2 skip / 17 pending), fitted/flat/encasement formula docs, standard_prices D1 schema, file structure, quote flow documentation, anti-spam docs, Phase 4 notes expanded |
