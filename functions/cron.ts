...
// Recommended schedule: 0 10 * * * (10:00 AM daily)
...
async function sendRecoveryEmail(
  ...
  try {
    const resp = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: env.ORDER_FROM_EMAIL || env.RECOVERY_FROM_EMAIL || 'MildMate <orders@mildmate.com>',
        to: [to],
        subject: subject,
        html: html,
      }),
...
async function processStage(
  ...
  if (stage === 1) {
    ...
    // Decide next stage based on basket size
    if (isHighValue) {
  ...
  if (stage === 2 && config.stage2Enabled && isHighValue) {
    ...
    // Insert discount into claims table
    try {
      ...
      console.log(`CRON: discount claim insert for ${cart.email} skipped (${e.message})`);
    }
...
export async function scheduled(event: ScheduledEvent, env: Env, ctx: ExecutionContext): Promise<void> {
  ...
  console.log(`CRON: config loaded — threshold=$${config.basketThresholdUsd}, stage2=${config.stage2Enabled}(${config.stage2Discount}%), stage3=${config.stage3Enabled}(${config.stage3Discount}%)`);
  ...
  // ── Stage 1: carts >24h, never contacted ──
  const stage1Limit = Math.min(remaining, 5);
  const { results: stage1Carts } = await env.DB.prepare(
    `SELECT id, email, cart_json, created_at FROM abandoned_carts
  ...
  if (stage1Carts && stage1Carts.length > 0) {
    console.log(`CRON: stage 1 — ${stage1Carts.length} cart(s) found`);
    for (const cart of stage1Carts as any[]) {
      const ok = await processStage(env, config, 1, cart);
      if (ok) sent++; else failed++;
    }
  }

  // ── Stage 2: carts where stage 1 was sent >48h ago ──
  if (sent < remaining && config.stage2Enabled) {
    const stage2Limit = Math.min(remaining - sent, 3);
    const { results: stage2Carts } = await env.DB.prepare(
      `SELECT id, email, cart_json, recovery_sent_at FROM abandoned_carts
       WHERE recovered = 0 AND recovery_stage = 1
         AND recovery_sent_at < datetime('now', '-48 hours')
       LIMIT ${stage2Limit}`
    ).all();

    if (stage2Carts && stage2Carts.length > 0) {
      console.log(`CRON: stage 2 — ${stage2Carts.length} cart(s) found`);
      for (const cart of stage2Carts as any[]) {
  ...
  // ── Stage 3: carts where stage 2 was sent >4 days ago ──
  if (sent < remaining && config.stage3Enabled) {
    const stage3Limit = Math.min(remaining - sent, 3);
    const { results: stage3Carts } = await env.DB.prepare(
      `SELECT id, email, cart_json, discount_code FROM abandoned_carts
       WHERE recovered = 0 AND recovery_stage = 2
         AND recovery_sent_at < datetime('now', '-4 days')
       LIMIT ${stage3Limit}`
    ).all();

    if (stage3Carts && stage3Carts.length > 0) {
      console.log(`CRON: stage 3 — ${stage3Carts.length} cart(s) found`);
      for (const cart of stage3Carts as any[]) {
