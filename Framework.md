...
- Clerk auth status reconciliation (2026-09-03): live `https://www.mildmate.com/js/clerk.js` serves a Development publishable key (`pk_test_*`), and current Clerk dashboard has no Production instance yet.
- Admin access reconciliation (2026-09-03): `norrawich.rat@gmail.com` added to `ADMIN_EMAILS` in both Cloudflare Pages environments, set to Clerk `public_metadata.role = "super-admin"` (current Development instance), and added to Cloudflare Zero Trust allow policies for `/admin/` and `/super-admin/`.
...
|---|---|---|
| Marine & Yacht | `/marine/` | Marine Fitted Sheet, Marine Top Sheet, Duvet Cover — Marine, 6-Sided Mattress Encasement, Pillowcases, Pillow Protector |
...
**Cloudflare Access status:** Zero-trust policies are now configured for `/admin/` and `/super-admin/` allowlists (defense-in-depth active). Option B can still be expanded as needed.
...
### Turnstile Bot Protection (Reconciled 2026-07-01)
- Cloudflare Turnstile is now enforced on contact + quote submissions.
- Frontend integration points: `public/contact/index.html`, `public/th/contact/index.html`, and quote modal in `public/js/product-configurator.js` (site key widget + token forwarding).
- Backend verification points: `workers/api/contact.ts` and `workers/api/quote.ts` must verify token via Cloudflare Siteverify using `TURNSTILE_SECRET_KEY`.
- Runtime parity is required after edits: sync Worker source with `public/index.js` and `public/_worker.js` to avoid stale validation behavior in production.
