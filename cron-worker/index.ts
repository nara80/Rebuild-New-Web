...
export default {
  async scheduled(event: ScheduledEvent, env: Env, ctx: ExecutionContext): Promise<void> {
    // UTC hour gate — only run during the configured hour
    const now = new Date();
    const nowUtcHour = now.getUTCHours();

    let configuredHour = 10; // default 10 AM UTC
    try {
      const row = await env.DB.prepare("SELECT value FROM recovery_config WHERE key = 'cron_hour_utc'").first();
      if (row && (row as any).value) configuredHour = Number((row as any).value) || 10;
    } catch {}

    if (nowUtcHour !== configuredHour) {
      console.log(`CRON: skipping — UTC hour ${nowUtcHour} != configured ${configuredHour}`);
      return;
    }
    ...
    if (!carts || carts.length === 0) {
      console.log('CRON: no eligible carts');
      return;
    }

    console.log(`CRON: ${carts.length} cart(s) found`);

    let sent = 0, failed = 0;

    for (const cart of carts as any[]) {
      let cartItems: CartItem[] = [];
      try { cartItems = JSON.parse(cart.cart_json || '[]'); } catch { continue; }
