...
// Standalone Worker deployed separately from Pages
// Runs daily at 10 AM via cron trigger configured in wrangler.toml
// Shares the same D1 database and Resend API key as the Pages project
...
async function loadRecoveryConfig(db: D1Database): Promise<RecoveryConfig> {
  ...
  try {
    ...
    console.log('CRON: recovery_config table not found, using defaults —', e.message);
    return defaults;
  }
}
...
async function processStage(env: Env, config: RecoveryConfig, stage: number, cart: any): Promise<boolean> {
  ...
  if (stage === 1) {
    ...
    console.log(`CRON: stage 1 sent to ${cart.email} ($${total.usd}, highValue=${isHighValue})`);
    return true;
  }
  ...
  if (stage === 2 && config.stage2Enabled && isHighValue) {
    ...
    try {
      await env.DB.prepare(
        `INSERT INTO discount_claims (code, email, status, discount_pct, expires_at, source, created_at)
         VALUES (?, ?, 'issued', ?, datetime('now', '+${config.discountExpiryDays} days'), 'abandoned_cart', datetime('now'))`
      ).bind(discountCode, cart.email, config.stage2Discount).run();
    } catch (e: any) { console.log(`CRON: discount insert skipped (${e.message})`); }
...
export default {
  async scheduled(event: ScheduledEvent, env: Env, ctx: ExecutionContext): Promise<void> {
    ...
    // Gate: only run at configured UTC hour
    let cronHour = 10;
    try {
      const hourRow = await env.DB.prepare("SELECT value FROM recovery_config WHERE key = 'cron_hour_utc'").first();
      if (hourRow) cronHour = parseInt((hourRow as any).value) || 10;
    } catch {}
    const nowHour = new Date().getUTCHours();
    if (nowHour !== cronHour) {
      console.log(`CRON: skipping — current hour ${nowHour} != configured ${cronHour}`);
      return;
    }

    const config = await loadRecoveryConfig(env.DB);
    console.log(`CRON: threshold=$${config.basketThresholdUsd}, s2=${config.stage2Enabled}(${config.stage2Discount}%), s3=${config.stage3Enabled}(${config.stage3Discount}%)`);

    const MAX_PER_DAY = 50;
    const todayCount = await env.DB.prepare("SELECT COUNT(*) as cnt FROM abandoned_carts WHERE recovery_sent_at >= datetime('now','start of day') AND recovery_sent_at IS NOT NULL").first();
    const sentToday = (todayCount && (todayCount as any).cnt) ? Number((todayCount as any).cnt) : 0;
    if (sentToday >= MAX_PER_DAY) { console.log(`CRON: daily limit ${sentToday}/${MAX_PER_DAY}`); return; }
    const remaining = MAX_PER_DAY - sentToday;

    let sent = 0, failed = 0;

    // Stage 1
    const s1 = await env.DB.prepare("SELECT id, email, cart_json FROM abandoned_carts WHERE recovered=0 AND recovery_stage=0 AND created_at < datetime('now','-24 hours') LIMIT " + Math.min(remaining, 5)).all();
    if (s1.results && s1.results.length > 0) {
      for (const cart of s1.results as any[]) { if (await processStage(env, config, 1, cart)) sent++; else failed++; }
    }

    // Stage 2
    if (sent < remaining && config.stage2Enabled) {
      const s2 = await env.DB.prepare("SELECT id, email, cart_json FROM abandoned_carts WHERE recovered=0 AND recovery_stage=1 AND recovery_sent_at < datetime('now','-48 hours') LIMIT " + Math.min(remaining - sent, 3)).all();
      if (s2.results && s2.results.length > 0) {
        for (const cart of s2.results as any[]) { if (await processStage(env, config, 2, cart)) sent++; else failed++; }
      }
    }

    // Stage 3
    if (sent < remaining && config.stage3Enabled) {
      const s3 = await env.DB.prepare("SELECT id, email, cart_json, discount_code FROM abandoned_carts WHERE recovered=0 AND recovery_stage=2 AND recovery_sent_at < datetime('now','-4 days') LIMIT " + Math.min(remaining - sent, 3)).all();
      if (s3.results && s3.results.length > 0) {
        for (const cart of s3.results as any[]) { if (await processStage(env, config, 3, cart)) sent++; else failed++; }
      }
    }

    console.log(`CRON: done — ${sent} sent, ${failed} failed (quota: ${sentToday + sent}/${MAX_PER_DAY})`);
  }
};
